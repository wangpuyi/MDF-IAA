...
class MANIQA(nn.Module):
    def __init__(self, embed_dim=768, num_outputs=1, patch_size=8, drop=0.1, 
        ...
        self.vit = timm.create_model('vit_base_patch8_224', pretrained=True, pretrained_cfg_overlay=dict(file='/home/wangpuyi/.cache/huggingface/hub/models--timm--vit_base_patch8_224.augreg2_in21k_ft_in1k/snapshots/9c48fc9fd87ab0a53f3368f1f7bccb20fca62aad/model.safetensors'))
    ...
    def forward(self, x):
        ...
        #     f = self.fc_score(x[i]) #[784, 384]
        #     print("f_size: ",f.shape) # torch.Size([784, 1])
        ...
        features = []
        for i in range(x.shape[0]):
            f = self.fc_score(x[i])  # [784, 384] -> [784, 1]
            w = self.fc_weight(x[i]) # [784, 384] -> [784, 1]
            feature = torch.cat((f, w), dim=0)  # Concatenating along the 0th dimension results in [1568, 1]
            features.append(feature.squeeze())  # Remove the singleton dimension and collect the feature

        # Stack the list of features along a new 0th dimension to get a tensor of shape [batch_size, 1568]
